...
function gjSameCfg(a, b){
  return !!a && !!b && a.lang === b.lang && a.style === b.style && a.length === b.length && a.texting === b.texting &&
    ...
    (a.formality || 3) === (b.formality || 3) && (a.conv || "") === (b.conv || "");
}
...
var GJ_RULES =
  ...
  "Real texting is rarely one smooth sentence stitched together with commas or 'and'. The moment a reply has more than one distinct beat — " +
  ...
  "2-3 short separate lines (joined by \\n in the same field) whenever a reply has more than one beat — stay on one line only when there is " +
  "genuinely just a single thought with nothing else attached.";
...
function gjReplySystem(style, relation, strength, emotion, formality){
  return GJ_RULES + "\n\n" +
...
function gjRunGenerate(){
  ...
  if(mode === "translate"){
    ...
    cfg = { lang:S.gj.lang, style:S.gj.style, length:S.gj.length, texting:S.gj.texting,
      relation:S.gj.relation, strength:S.gj.strength, emotion:S.gj.emotion, formality:S.gj.formality,
      conv: mode === "reply" ? S.gj.activeConv : "" };
    sys = mode === "reply" ? gjReplySystem(S.gj.style, S.gj.relation, S.gj.strength, S.gj.emotion, S.gj.formality)
                           : gjRewriteSystem(S.gj.style, S.gj.relation, S.gj.strength, S.gj.emotion, S.gj.formality);
...
/* Language/Style/Talking-to/Emotion/Texting/Boldness are native <select>
...
function gjCfgHTML(){
  return '<div class="cfgsection">' +
    ...
    '<div class="cfgsection">' +
      ...
      gjSelectRow("Texting style", GJ_TEXTING, S.gj.texting, "texting") +
