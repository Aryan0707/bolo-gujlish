...
function gjBurstRule(burst){
  ...
  var rule = "Real texting is rarely one smooth sentence stitched together with commas or 'and'. The moment a reply has more than one distinct beat — " +
    ...
    "Never join two clauses inside one line with a comma or a connector like 'ke' (or) either — that comma is itself the seam between two beats, " +
    "so split there instead. For example write 'Tare kai kaam hatu' as its own line, then 'ke bas vaat karvi hati?' as the next line — never " +
    "'Tare kai kaam hatu, ke bas vaat karvi hati?' joined together on one line with a comma.";
  if(burst === "longer")
