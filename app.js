...
function go(name){
  ...
  if(gjSheetOpen || gjModelSheetOpen) gjUnlockScroll();
  gjSheetOpen = null;
  gjModelSheetOpen = false;
  screen = name;
  paintShell();
...
var gjModelSheetOpen = false;  /* Settings' own model picker sheet — separate screen, separate state */

/* A sheet is a fixed-position overlay, but without this the page behind
   it still scrolls along with a touch-drag inside the sheet's own list
   on mobile, and a full re-render on open/select/close can otherwise
   leave you scrolled somewhere you weren't. Locks the body in place at
   its current scroll offset while a sheet is open, and restores that
   exact offset — not the top of the page — once it closes. */
var gjScrollLockY = 0;
function gjLockScroll(){
  gjScrollLockY = window.scrollY;
  document.body.style.position = "fixed";
  document.body.style.top = "-" + gjScrollLockY + "px";
  document.body.style.width = "100%";
}
function gjUnlockScroll(){
  document.body.style.position = "";
  document.body.style.top = "";
  document.body.style.width = "";
  window.scrollTo(0, gjScrollLockY);
}
    ...
    '<div class="sheet panelenter">' +
      ...
      '<div class="sheetlist">' + def.list().map(function(o){
        ...
        return '<button class="sheetopt' + (on ? " on" : "") + '" data-sheetval="' + o.id + '">' +
          '<span' + (looksGujScript(o.lab) ? ' class="guj"' : "") + '>' + esc(o.lab) + '</span>' + (on ? gjIcon("check", 16) : "") +
        '</button>';
      }).join("") + '</div>' +
    '</div>' +
  '</div>';
}
/* Settings' own model sheet — same sheetopt/sheetoverlay styling as the
   Translate-screen config sheets, but scoped to S.ai.model on a
   different screen, so it isn't part of the GJ_SHEETS/gjSheetOpen
   system above. Keeps the same verified/"not yet verified" split the
   native <select> used to show via optgroup. */
function gjModelSheetHTML(models){
  var verified = models.filter(function(m){ return m.cost !== "Unverified"; });
  var unverified = models.filter(function(m){ return m.cost === "Unverified"; });
  function optBtn(m){
    var on = S.ai.model === m.id;
    return '<button class="sheetopt' + (on ? " on" : "") + '" data-modelval="' + esc(m.id) + '">' +
      '<span>' + esc(m.lab) + (m.cost !== "Unverified" ? ' <span class="sheetopt-cost">— ' + esc(m.cost) + '</span>' : "") + '</span>' +
      (on ? gjIcon("check", 16) : "") +
    '</button>';
  }
  return '<div class="sheetoverlay" id="gjModelSheetOverlay">' +
    '<div class="sheet panelenter">' +
      '<div class="sheethead"><span>Model</span><button class="linkbtn sm" id="gjModelSheetDone">Done</button></div>' +
      '<div class="sheetlist">' +
        '<div class="cfgsectionlab" style="padding:8px 12px 4px">Models</div>' +
        verified.map(optBtn).join("") +
        (unverified.length ? '<div class="cfgsectionlab" style="padding:14px 12px 4px">Not yet verified</div>' + unverified.map(optBtn).join("") : "") +
      '</div>' +
    '</div>' +
  '</div>';
}
...
function gjWireSheets(){
  each($$("[data-sheet]"), function(b){
    b.onclick = function(){ gjSheetOpen = b.getAttribute("data-sheet"); gjLockScroll(); sfxTap(); renderTranslate(); };
  });
  var overlay = $("#gjSheetOverlay");
  if(!overlay) return;
  overlay.onclick = function(e){ if(e.target === overlay){ gjSheetOpen = null; gjUnlockScroll(); sfxTap(); renderTranslate(); } };
  var done = $("#gjSheetDone");
  if(done) done.onclick = function(){ gjSheetOpen = null; gjUnlockScroll(); sfxTap(); renderTranslate(); };
...
function renderSettings(){
  ...
  '<div class="glass pad" style="margin-bottom:14px">' +
    ...
    '</div>' +
    '<div class="field">' +
      gjCfgRow("Model", gjLab(models, S.ai.model), "aimodel") +
      '<div class="hint" id="mnote">' + esc((models.filter(function(m){ return m.id === S.ai.model; })[0] || {}).note || "") + '</div>' +
    '</div>' +
    '<div class="rowbtns">' +
  ...
  '<div class="glass pad">' +
    ...
    '</div>' +
  '</div>' +
  (gjModelSheetOpen ? gjModelSheetHTML(models) : "");

  $("#sndOn").onclick = function(){ S.sound = true; save(); sfxTap(); renderSettings(); };
  $("#sndOff").onclick = function(){ S.sound = false; save(); renderSettings(); };

  var modelRow = $('[data-sheet="aimodel"]');
  if(modelRow) modelRow.onclick = function(){ gjModelSheetOpen = true; sfxTap(); renderSettings(); };
  var modelOverlay = $("#gjModelSheetOverlay");
  if(modelOverlay){
    modelOverlay.onclick = function(e){ if(e.target === modelOverlay){ gjModelSheetOpen = false; sfxTap(); renderSettings(); } };
    var mDone = $("#gjModelSheetDone");
    if(mDone) mDone.onclick = function(){ gjModelSheetOpen = false; sfxTap(); renderSettings(); };
    each($$("[data-modelval]"), function(b){
      b.onclick = function(){
        S.ai.model = b.getAttribute("data-modelval");
        gjModelSheetOpen = false;
        save(); sfxTap();
        renderSettings();
      };
